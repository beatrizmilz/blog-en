...
[^1]: **A conta graduita tem algumas limitações, como o número de linhas de código geradas por mês. É possível consultar mais informações na [página de planos do Copilot](https://github.com/features/copilot/plans).**
...
Ao abrir a opção *Immersive*, o Copilot é aberto em uma nova aba, onde podemos acessar um [chat com o Copilot](https://github.com/copilot). Se você tem costume de utilizar o ChatGPT, o Copilot na interface web é bem similar!
...
Para quem usa o Visual Studio Code, você precisa instalar a extensão do Copilot:
...
Observe que ainda assim não é perfeito: eu costumo utilizar o pipe nativo (`|>`) e as sugestões feitas foram com o pipe do pacote `magrittr` (`%>%`). Em scripts onde eu já tenha escrito mais código, o Copilot tende a acertar mais sugestões e utilizar o pipe que eu costumo utilizar, pois ele utiliza o meu código como contexto.
